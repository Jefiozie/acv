...
| Property | Value |
|----------|-------|
| Framework | **Vitest** + `@analogjs/vitest-angular` (replaces Angular CLI default Karma) |
| Config file | `frontend/vitest.config.ts` |
| Quick run command | `npm run test` (in `frontend/`) |
| Full suite command | `npm run test -- --coverage` |

> **Note:** Angular CLI 21 generates Karma by default. For this project, `karma` and `jasmine-core` are removed post-scaffold and replaced with `vitest` + `@analogjs/vitest-angular` + `@testing-library/angular`. The `@analogjs/vitest-angular` plugin is **required** for Angular decorator and template compilation inside Vitest — without it, component specs fail to compile.
...
- [ ] `frontend/vitest.config.ts` — Vitest config with `@analogjs/vitest-angular` plugin (Wave 0: scaffold)
...
1. **Complete ACV Groep township list**
   ...
   - RESOLVED: Hardcode only `{ id: "16", name: "Ede" }` for v1. Add `// TODO: add more townships when ACV expands` comment in the model file.
...
2. **Signal Forms vs ReactiveFormsModule risk tolerance**
   ...
   - RESOLVED: User accepts experimental risk. Proceed with Signal Forms, pin Angular to exactly `21.2.14` (no `^` or `~`).
...
3. **Phase 2 API URL for local dev**
   ...
   - RESOLVED: Default to mock interceptor (`useMockApi: true` in `environment.development.ts`). No SAM CLI required. Switch to real API when Phase 2 is deployed.
